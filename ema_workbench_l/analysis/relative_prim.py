...
class Prim(sdutil.OutputFormatterMixin):
    ...
    def _update_yi_remaining_weighted(self):
        '''
    ...
    def _peel(self, box):
        ...
        for entry in possible_peels:
            i, box_lim = entry
            try:
                obj = self.obj_func(self, self.y[box.yi], self.y[i], self.relative_likelihood[box.yi], self.relative_likelihood[i])
            except:
    ...
    def _weighted_obj_func(self, y_old, y_new, weight_old, weight_new):
        r'''
        lenient objective function modified to take into account relative weight of each state of the world
        '''
        mean_old = np.mean(y_old * weight_old)

        if y_new.shape[0] > 0:
            mean_new = np.mean(y_new * weight_new)
        else:
            mean_new = 0
    ...
    _obj_functions = {PRIMObjectiveFunctions.LENIENT2: _lenient2_obj_func,
                      ...
                      PRIMObjectiveFunctions.WEIGHTED: _weighted_obj_func}

    _update_functions = {'default': _update_yi_remaining_default,
                         'guivarch': _update_yi_remaining_guivarch}


def _new_obj_fucntion_prim(self, y_old, y_new):

	# your code goes here
	self.relative_likelihood

class RelativePrim(Prim):

	def __init__(self, relative_likelihood, x, y, threshold,
                 obj_function=PRIMObjectiveFunctions.WEIGHTED,
                 peel_alpha=0.05, paste_alpha=0.05, mass_min=0.05,
                 threshold_type=ABOVE, mode=sdutil.RuleInductionType.BINARY,
                 update_function='default'):
		self.relative_likelihood = relative_likelihood
		super().__init__(x, y, threshold,
                 obj_function=obj_function,
                 peel_alpha=peel_alpha, paste_alpha=paste_alpha, mass_min=mass_min,
                 threshold_type=threshold_type, mode=mode,
                 update_function=update_function)

#def some_new_update_function(self, box_lims, indices):
	#your new update function goes here
...
#PrimBox.update = some_new_update_function
